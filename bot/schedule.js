...
const biliBind = require('./uniControllers/biliBind');
...
module.exports = {
    ...
    biliCheck: schedule.scheduleJob(`*/${interval} * * * * *`, async () => {
        ...
        // 获取列表
        let liveroomList = dataDB.prepare(`SELECT * FROM liverooms;`).all();
        ...
        for (let i = 0; i < liveroomList.length; i++) {
            ...
            if (livingItem.change && livingItem.status) {
                let info = await biliBind.getUserInfo(livingItem.uid);
                if (info.code) return;
        ...
        for (let i = 0; i < liveroomList.length; i++) {
            ...
            if (
                ...
                // 获取群聊列表
                let groupList = (await biliBind.getLiveroomDetail(liveroomItem.uid)).data;
